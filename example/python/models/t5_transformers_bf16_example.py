...
from transformers import T5Model, T5Tokenizer, BartModel, T5ForConditionalGeneration, AutoTokenizer
...
# model_dir = "/root/data/models/0906_ttg_t5_bug/"
# model_dir = "/root/data/models/2023/0201_t5_bf16/"
# model_dir = "/home/0906_ttg_t5_bug/"
# model_dir = "/home/0928_ttg_t5_fp16_nan/"
model_dir = "/home/0201_t5_bf16/"
...
def test(batch_size=4, prompt_seq_len=8, max_seq_len=1024, ts_data_type=torch.float32, eet_data_type=torch.float32, using_eet=True, using_ts=True, loop=100):
    ...
    encoder_seq_len = torch.tensor([prompt_seq_len] * batch_size).int().cuda()
    ...
    if using_ts:
        ...
        ts_model = ts_model.to(ts_data_type).cuda()
        ...
        for i in range(loop):
            ...
            for j in range(max_seq_len):
                with torch.no_grad():
                    res_ts = ts_model(input_ids=input_ids, decoder_input_ids=decoder_input_ids, past_key_values=past_key_values, attention_mask=attention_mask, encoder_outputs=encoder_outputs, use_cache=True)
                if j == (max_seq_len - 1) and i == (loop - 1):
                    print(j, ' res ts: ', res_ts.last_hidden_state[:,0,::10], ' shape: ', res_ts.last_hidden_state.size())
    ...
    if using_eet:
    # load model
        eet_model = EETT5Model.from_pretrained(model_dir, batch_size, max_prompt_seq_len=prompt_seq_len, max_full_seq_len=max_seq_len, data_type=eet_data_type)
        ...
        for i in range(loop):
            ...
            for j in range(max_seq_len):
                ...
                if j == (max_seq_len - 1) and i == (loop - 1):
                    print(j, ' res eet: ', res_eet[0][:,0,::10], ' shape: ', res_eet[0].shape)
                if first_pass:
                    first_pass = False
                input_ids = input_inc_decoder2
                decoder_input_ids = input_inc_decoder2
    ...
    print('*****************************')
    print(batch_size, '\t', prompt_seq_len, '\t', max_seq_len, ' eet data type: ', eet_data_type, ' ts data type: ', ts_data_type)
...
def test_text_generate():
    ...
    model = T5ForConditionalGeneration.from_pretrained(model_dir, from_flax=False).to('cuda:0').to(torch.bfloat16)
    ...
    inputs = tokenizer([input_str] * 1, return_tensors='pt', add_special_tokens=False)
    ...
    setup_seed(1)
    eet_model = EETT5ForConditionalGeneration.from_pretrained(model_dir, max_batch=1, data_type=torch.float32)
    #eet
    torch.cuda.synchronize()
    t1 = time.perf_counter()
...
if __name__ == '__main__':
    max_seq_len = 50
    # test(batch_size=10, seq_len=200, max_seq_len=max_seq_len,
    #      ts_data_type=torch.float32, eet_data_type=torch.float32, using_eet=True, using_ts=True, loop=10)
    test(batch_size=10, prompt_seq_len=1, max_seq_len=max_seq_len,
         ts_data_type=torch.bfloat16, eet_data_type=torch.bfloat16, using_eet=True, using_ts=False, loop=10)
    # test(batch_size=10, seq_len=200, max_seq_len=max_seq_len,
    #      ts_data_type=torch.float16, eet_data_type=torch.float16, using_eet=True, using_ts=True, loop=10)
    # test_text_generate()