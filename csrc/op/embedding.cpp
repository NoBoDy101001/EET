...
namespace eet
{
    namespace op
    {
        ...
        emb_cache_name_(emb_cache_name)
        {
            ...
            // Buffer& emb_ffn_out = MManager::get_instance().get_cache(desc_.batch_size_ * desc_.max_full_seq_len_ * desc_.hidden_units_, desc_.dtype_, desc_.options_, emb_cache_name_+desc_.device_name);
        }
        ...
        torch::Tensor Embedding::forward_fairseq(const torch::Tensor &input_tensor,const torch::Tensor &positions_mask,bool no_scale_embedding,int padding_idx,bool first_pass)
        {
            ...
            // printf("step:%d\n",step_);
            Buffer& output = MManager::get_instance().get_cache(desc_.batch_size_ * desc_.max_full_seq_len_ * desc_.hidden_units_, desc_.dtype_, desc_.options_, emb_cache_name_+desc_.device_name);
